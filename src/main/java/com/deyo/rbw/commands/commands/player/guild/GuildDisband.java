...
public class GuildDisband {
    public static void execute(String[] args2, Guild g2, Member m3, MessageChannelUnion c, CommandAdapter msg, String usage) {
        ...
        if (!guild.getLeader().equalsIgnoreCase(m3.getId())) {
            BetterEmbed error = new BetterEmbed("error", "Error", "", "You are not the guild leader. Please use =guild leave to leave the guild!", "");
            error.reply(msg);
            return;
        }
        ...
        guild.disband();
        BetterEmbed error = new BetterEmbed("success", "Success", "", "You disbanded the guild **" + guild.getGuildName() + "**!", "");
        error.reply(msg);
    }
}
