...
implements ServerCommand {
    @Override
    public void doCMD(String[] args, Guild guild, Member member, MessageChannelUnion ch, CommandAdapter msg, String usage) {
        
        BetterEmbed embed = new BetterEmbed("info", "\uD83D\uDCDD  RBW Bot Help", guild.getIconUrl() == null ? "" : guild.getIconUrl(), "Choose a category below to view its commands.", "deyo" + com.deyo.rbw.Main.version);
        
        // Quick overview fields
        
        StringSelectMenu menu = StringSelectMenu.create("help-" + member.getId()).setPlaceholder("Select command group...")
                .addOptions(
