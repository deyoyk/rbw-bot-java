    ...
    public void doCMD(String[] args2, Guild g2, Member m3, MessageChannelUnion c, CommandAdapter msg, String usage) {
        if (args2.length == 4) {
            try {
                ...
                BetterEmbed embed1 = new BetterEmbed("default", "Command sent", "",
                        "**" + (String) content + "**\nBy " + m3.getAsMention(), "");
                ...
                if (!Player.isPlayer(ID2)) {
                    ...
                    Player.createFile(ID2, playerName);
                }
                playerName = Player.getName(ID2);
                if (playerName == null) {
                    playerName = member.getEffectiveName();
                }
                BetterEmbed embed = new BetterEmbed("success", "Modified " + playerName + "'s Stats", "", "", "");
                ...
                embed.addField("player's " + statistic.getPath(), oldstat + " > " + (int) statistic.getForPlayer(ID2),
                        true);
                Player.fix(ID2, g2);
                embed.reply(msg);
            } catch (Exception e) {
