...
import com.deyo.rbw.childclasses.configuration.serialization.DelegateDeserialization;
import com.deyo.rbw.childclasses.configuration.serialization.SerializableAs;
...
public class ConfigurationSerialization {
    ...
    protected ConfigurationSerializable deserializeViaMethod(Method method, Map<String, ?> args2) {
        try {
            ConfigurationSerializable result = (ConfigurationSerializable)method.invoke(null, args2);
            if (result != null) {
                return result;
            }
    ...
    protected ConfigurationSerializable deserializeViaCtor(Constructor<? extends ConfigurationSerializable> ctor, Map<String, ?> args2) {
        try {
    ...
    public ConfigurationSerializable deserialize(Map<String, ?> args2) {
        Constructor<? extends ConfigurationSerializable> constructor;
        Validate.notNull(args2, "Args must not be null", new Object[0]);
        ConfigurationSerializable result = null;
        Method method = null;
        method = this.getMethod("deserialize", true);
        if (method != null) {
            result = this.deserializeViaMethod(method, args2);
        }
        if (result == null && (method = this.getMethod("valueOf", true)) != null) {
            result = this.deserializeViaMethod(method, args2);
        }
        if (result == null && (constructor = this.getConstructor()) != null) {
            result = this.deserializeViaCtor(constructor, args2);
        }
        return result;
    }

    public static ConfigurationSerializable deserializeObject(Map<String, ?> args2, Class<? extends ConfigurationSerializable> clazz) {
        return new ConfigurationSerialization(clazz).deserialize(args2);
    }

    public static ConfigurationSerializable deserializeObject(Map<String, ?> args2) {
        Class<? extends ConfigurationSerializable> clazz = null;
        if (args2.containsKey(SERIALIZED_TYPE_KEY)) {
            try {
                String alias = (String)args2.get(SERIALIZED_TYPE_KEY);
                if (alias == null) {
    ...
    public static String getAlias(Class<? extends ConfigurationSerializable> clazz) {
        ...
        if ((alias = clazz.getAnnotation(SerializableAs.class)) != null && alias.value() != null) {
            return alias.value();
        }
