...
public class ButtonsEvent
    ...
    public void onChannelDelete(@NotNull ChannelDeleteEvent event) {
        ...
        for (ScreenShare screenShare : ScreenShare.screenShares) {
            if (!screenShare.getChannelID().equalsIgnoreCase(channelID))
                continue;
            screenShare.delete();
            break;
        }
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (event.getButton().getId().contains("GAMES-")) {
            ...
            ((MessageEditAction) event.getMessage().editMessageEmbeds(Games.showGames(ID2, page, state).build())
                    .setActionRow(Games.buttons(ID2, state))).queue();
        }
        if (event.getButton().getId().equals("previousLB-" + event.getMember().getId())) {
            ...
            GuildJoin.execute(new String[] { "=guild", "join" }, event.getGuild(), event.getMember(),
                    event.getChannel(), new CommandAdapter(event.getMessage()), "");
        } else if (event.getButton().getId().equals("accept-" + event.getMember().getId())) {
            event.getChannel().sendMessage(event.getMember().getAsMention()).queue();
            PartyJoin.execute(new String[] { "=party", "join" }, event.getGuild(), event.getMember(),
                    event.getChannel(), new CommandAdapter(event.getMessage()), "");
            ...
            try {
                ...
                Strike.strike(targetId, event.getGuild(), event.getMember(), targetMember, reason,
                        new CommandAdapter(event.getMessage()));
            ...
            BetterEmbed error1 = new BetterEmbed("success", "Ranked BedWars Moderation", "", "<@" + ID2
                    ...
                    "");
            ((MessageCreateAction) event.getMessage().reply("<@" + ID2 + ">, " + event.getMember().getAsMention())
                    .setEmbeds(error1.build())).queue();
            Statistic.SCREENSHARED.setForPlayer(event.getMember().getId(),
                    Statistic.SCREENSHARED.getForPlayer(event.getMember().getId()) + 1.0);
            ...
            event.getMessage().reply(event.getMember().getAsMention()
                    + ", You denied the SS Request\n :wastebasket: \ufe0f Transcript saving, This channel will be deleted in 10 seconds")
                    .queue();
            ...
            BetterEmbed embed1 = new BetterEmbed("default", "SS Closed", "",
                    "Reason: **Denied by **" + event.getMember().getAsMention() + "\nTarget: <@" + ID2 + ">", "");
    ...
    public void previouslbpage(final Message message, String ID2) {
        ...
        try {
            if (footerText.contains("**")) {
                String pageStr = footerText.replaceAll("[^0-9]", "");
        ...
        if (page != 1) {
            ...
            if ((page - 1) * 10 < list.size() && page > 0) {
                ...
                for (int i = (page - 1) * 10; i < page * 10; ++i) {
                    ...
                    int place = i + 1;
                    lb = place == 1
                            ? (String) lb + ":first_place: `" + Player.getName(values2[0]) + "` \u2014 "
                                    + formatter.format(Double.parseDouble(values2[1])) + "\n"
                            : (place == 2
                                    ? (String) lb + ":second_place: `" + Player.getName(values2[0]) + "` \u2014 "
                                            + formatter.format(Double.parseDouble(values2[1])) + "\n"
                                    : (place == 3
                                            ? (String) lb + ":third_place: `" + Player.getName(values2[0]) + "` \u2014 "
                                                    + formatter.format(Double.parseDouble(values2[1])) + "\n"
                                            : (String) lb + "**#" + place + "** `" + Player.getName(values2[0])
                                                    + "` \u2014 " + formatter.format(Double.parseDouble(values2[1]))
                                                    + "\n"));
                }
                BetterEmbed embed = new BetterEmbed("info", "\uD83C\uDFC6  " + Utils.formatName(stat) + " Leaderboard",
                        "", (String) lb, "Page **" + page + "**");
                ((MessageEditAction) message.editMessageEmbeds(embed.build()).setActionRow(ButtonsEvent.buttons(ID2)))
                        .queue();
            } else {
